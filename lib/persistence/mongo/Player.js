var Backbone = require('backbone');
var Q = require("q");
var mongo_promise = require('./client.js');
var ObjectId = require('mongodb').ObjectID;

module.exports = function(weiqi){

var BasePlayer = require('./Base')({model: Backbone.Model, name: 'player'});

weiqi.Player = BasePlayer.extend({

  collection_name: "players",
});

// class methods
weiqi.Player.find_by_board_id_and_color = function(board_id, color) {

  var deferred = Q.defer();

  var model = this;

  mongo_promise.then(function(db) {
    var collection = db.collection('players');
    console.log('looking up collection', collection, model.collection_name)
    collection.findOne({board_id: ObjectId(board_id), color: color}, function(err, result){
      console.log('In callback')
      if(err){
        console.log('error lookup up player')
        deferred.reject(err);
      }
      else{
        console.log('discovered player!', result)
        deferred.resolve(new weiqi.Player(result));
      }
    })
  }).fail(function(err){
    console.log('IN FAIL', err);
    deferred.reject(err)
  })
  return deferred.promise;
}


return weiqi

}
