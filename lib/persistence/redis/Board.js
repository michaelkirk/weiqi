...
module.exports = function(weiqi){
  ...
  weiqi.Move = require('../backbone_sync.js')(weiqi.Move)

  weiqi.Move = weiqi.Move.extend({
    pk: function(){
      return 'moves:' + this.id;
    },
    create: function(options){
      var deferred = Q.defer();
      var move = this;
      this.id = this.get('board_id') + ':' + this.get('num');
      redis_client.set(this.pk(), JSON.stringify(this.toJSON()), function(err, result) {
        if(err || result == null){
          deferred.reject(err)
        }
        else{
          deferred.resolve(move.toJSON());
        } 
      })
